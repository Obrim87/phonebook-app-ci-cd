...
const PersonForm = ({
  ...
  const addName = (e) => {
    ...
    if (
      ...
      if (
        window.confirm(
          `${nameObject.name} already exists, do you wish to update the phone number?`
        )
      ) {
        let changedPerson = persons.find(
          (p) => p.name.toLowerCase() === nameObject.name.toLowerCase()
        );
        ...
        personServices
          ...
          .then((response) => {
            setPersons(
              persons.map((p) =>
                p.name.toLowerCase() === response.data.name.toLowerCase()
                  ? response.data
                  : p
              )
            );
          ...
          .catch((error) => {
            if (error.response.status === 404) {
              setErrorMessage(
                `Information on ${changedPerson.name} has already been removed from the server.`
              );
  ...
  return (
    <form>
      <div>
        ...
        <br></br>
        <br></br>
        number: <input onChange={(e) => setNewTel(e.target.value)} />
        <br></br>
        <br></br>
        <button type='submit' onClick={addName}>
          {persons.find((e) => e.name.toLowerCase() === newName.toLowerCase())
            ? 'update'
            : 'add'}
        </button>
      </div>
